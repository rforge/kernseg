...
#include "KernDynProg_dD_CNBAF_last.h"

// Define the appropriate Kernal function


double kernel_dD_CNandBAF(double * x, double * y, int d, double * delta){
	double sum = 0.0;
	for(int t = 0; t < d; t++){
		sum = sum + exp( -(x[t] - y[t]) * (x[t] - y[t])/ delta[t]);
	}
...
#define KERN(x, y, delta)  (kernel_dD_CNandBAF( (x),  (y), *d_, delta ) )
...
void KernDynProg_dD_CNBAF_Raccess(double * vector_mat_i, int *n_, int *d_, int * K_, int *M_ij, double *C_ij, double *delta, double* D_i, int * min_size_){
	...
	KernDynProg_dD_CNBAF(x_i, n_, d_, K_, M_ij, C_ij, delta, D_i, min_size_);
